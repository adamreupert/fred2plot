...
#Import local files
import XRD_plotting_script

def widget_ui(mainwindow):
    
    # command block of triggered action of button presses
    ...
    def impdir_load():
        print('Import')
        currdir = os.getcwd()
        directory = fd.askdirectory(initialdir=currdir, title='Please select a import directory')
        if len(directory) > 0:
            directory = directory + '/'
            #print(directory)
            Label_inputdir_result['text'] = directory
        else:
    ...
    def expdir_load():
        ...
        if len(directory) > 0:
            directory = directory + '/'
            #print(directory)
            Label_exportdir_result['text'] = directory
        else:
            print('Nothing selected?')
    ...
    def data_generate():
        ...
        if Label_inputdir_result['text'] == 'Nothing selected':
            ...
            if formatchoice.get() == 1:
                print('Singleplot')
                if option_rename.get():
                    print("File is reformated")
                    XRD_plotting_script.rename(Label_inputdir_result['text'], Label_exportdir_result['text'])
                if option_baseline.get():
                    print("File is baseline substracted")
                    XRD_plotting_script.baseline_substraction(Label_inputdir_result['text'], Label_exportdir_result['text'])
                if option_norm.get():
                ...
                if option_pngexp.get():
                    ...
                    XRD_plotting_script.single_plot(Label_inputdir_result['text'], Label_exportdir_result['text'])
            elif formatchoice.get() == 2:
                print('Multiplot')
                if option_rename.get():
                    print("File is reformated")
                    #Fred_routine.rename_automatic(Label_inputdir_result['text'], Label_exportdir_result['text'])
                if option_baseline.get():
                    print("File is baseline substracted")
                ...
                print('Debug: Something is wrong')
...
#Right UI side
    ...
    Button_inputdir = Button(widgetgrid, text='Load', state = "normal", command = impdir_load)
    Button_exportdir = Button(widgetgrid, text='Export', state = "normal", command = expdir_load)
    ...
    Button_generate = Button(widgetgrid, text='Generate Data', state = "normal", command = data_generate)
    Button_update = Button(widgetgrid, text='Update plot', state = "normal", command = plot_update)
    Button_load = Button(widgetgrid, text='Load plot', state = "normal", command = plot_load)
...
#Right side
    ...
    Label_inputdir_result.grid(row=15,column=18, columnspan=8, sticky=N+S+W)
    Label_exportdir_result.grid(row=16,column=18, columnspan=8, sticky=N+S+W)
    widgetgrid.grid_rowconfigure(17, minsize=10)
    Button_generate.grid(row=18,column=15, columnspan=3, sticky=N+S+W+E)
    Button_update.grid(row=18,column=18, columnspan=4, sticky=N+S+W+E)
    Button_load.grid(row=18,column=22, columnspan=3, sticky=N+S+W+E)
